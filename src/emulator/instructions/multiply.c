...
short multiply(word *instruction, struct RegisterFile *registers, memory_t memory) {
    ...
    *rd = (*rm * *rs) + (accumulate ? *rn : 0); // if accumulate bit is set perform mla rather than mul instruction by adding the contents in rn
    // Update the cpu state with the value in rd (result)
    ...
    // If required, the cpu state is updated the flags
    if(to_set) {
